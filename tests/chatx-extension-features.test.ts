...
describe("ChatX unified browser extension", () => {
  ...
  it("gates Agent Bridge sends, rollover and replacement-tab creation while retaining terminal send guard", () => {
    ...
    expect(background).toContain("async function createClaimedAgentTab");
    expect(background).toContain("async function createRolloverTab");
    const createStart = background.indexOf("async function createClaimedAgentTab");
    ...
    const rolloverCreateBlock = background.slice(rolloverStart, rolloverEnd);
    expect(rolloverCreateBlock).toContain("createClaimedAgentTab({");
    expect(rolloverCreateBlock).not.toContain("chrome.tabs.create");
